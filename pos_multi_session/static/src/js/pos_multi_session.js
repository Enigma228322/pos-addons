...
odoo.define("pos_multi_session", function(require) {
    ...
    screens.OrderWidget.include({
        ...
        orderline_add: function() {
            var options = arguments && arguments[2] ? arguments[2] : {};
    ...
    models.PosModel = models.PosModel.extend({
        ...
        after_load_server_data: function() {
            ...
            $.when(res).then(function() {
                ...
                var load_sync_all_request = function() {
                    var response = self.multi_session.request_sync_all({
            ...
            if (this.multi_session) {
                if (reason === "finishOrder") {
                    if (this.get("orders").size() > 0) {
                        return this.set({
                            selectedOrder:
                                this.get("orders").at(index) ||
                                this.get("orders").first(),
                        });
                    }
                    this.add_new_order();
        ...
        updates_from_server_callback: function(message) {
            ...
            var same_session_check =
                get_attr(message, "session_id") === this.pos_session.id;
            var same_login_check =
                get_attr(message, "login_number") === this.pos_session.login_number;
            if (
                same_session_check &&
            ...
            try {
                ...
                if (action === "sync_all") {
                    ...
                    _.each(data.orders, function(received_data) {
                        order =
                            self.get("orders").find(function(item) {
        ...
        ms_on_add_order: function(current_order) {
            ...
            if (
                this.config.multi_session_replace_empty_order &&
                ...
                is_frozen ||
                !current_order.new_order ||
                !this.config.multi_session_deactivate_empty_order
            ) {
        ...
        ms_update_existing_order: function(order, data) {
            ...
            if (data.partner_id === false) {
                ...
                if (!client) {
                    $.when(this.load_new_partners_by_id(data.partner_id)).then(
                        function(new_client) {
            ...
            _.each(data.lines, function(dline) {
                ...
                if (!line) {
                    line = new models.Orderline(
                        {},
                        {pos: pos, order: order, product: product}
                    );
                    line.uid = dline.uid;
        ...
        ms_update_order: function(order, data) {
            ...
            if (order) {
                ...
                var create_new_order =
                    pos.config.multi_session_accept_incoming_orders ||
                    !(
                        data.ms_info &&
                        data.ms_info.created.user.id !== pos.ms_my_info().user.id
                    );
                if (!create_new_order) {
                    return;
                }
    ...
    chrome.OrderSelectorWidget.include({
        init: function(parent, options) {
            ...
            this.pos
                .get("orders")
                .bind("change:update_new_order", this.renderElement, this);
        },
        destroy: function() {
            this.pos
                .get("orders")
                .unbind("change:update_new_order", this.renderElement, this);
            this._super();
        },
        ...
        update_revision_id: function(res) {
            ...
            if (
                ...
                (this.sequence_number !== order_ID ||
                    this.pos.pos_session.sequence_number === order_ID)
            ) {
                this.sequence_number = order_ID;
                ...
                this.pos.pos_session.sequence_number = Math.max(
                    Boolean(sequence_number) && sequence_number,
                    order_ID + 1 || 1
                );
                // Rerender order
                this.trigger("change");
    ...
    exports.MultiSession = Backbone.Model.extend({
        ...
        bind_ms_connection_events: function() {
            var self = this;
            this.pos.sync_bus.longpolling_connection.on(
        ...
        send: function(message, options) {
            ...
            var send_it = function() {
                ...
                return session.rpc(
                    temp + "/pos_multi_session_sync/update",
                    {
